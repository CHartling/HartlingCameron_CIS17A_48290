...
int *mode(const int *array, int arySize) {
    ...
    int max_freq = 0; // initialize with zero
    int cur_element = -1; // current element
    int cur_freq = 0; // current element frequency

    // count identical elements while traversing and keep track of max Frequency
    for(int i = 0; i < arySize; ++i) {
        if(cur_element == ary[i]) { // like element
            cur_freq++; // increase cur frequency
            max_freq = max(cur_freq, max_freq); // take max
        }
        
        else { // distinct element
            cur_element = ary[i]; // change cur element
            cur_freq = 1; // update cur frequency
            max_freq = max(cur_freq, max_freq); // take max
        }
    }
    ...
    int nmodes;
    if(max_freq < 2) nmodes = 0;
    
    else {
        nmodes = 0; // initialize with zero
        cur_element = -1; // current element
        cur_freq = 0; // current element frequency
        for(int i = 0; i < arySize; ++i) {
            if(cur_element == ary[i]) { // like element
                cur_freq++; // increase cur frequency
            }
            else { // distinct element
                if(cur_freq == max_freq) nmodes++; // compare with max and increase nmodes
                    cur_element = ary[i]; // update cur element
                    cur_freq = 1; // update cur frequency
                }
            }
            if(cur_freq == max_freq) nmodes++; // for last element
    }

    //Dynamically Allocate the mode array
    int *modeAry = new int[nmodes + 2];
    modeAry[0] = nmodes; //returns nomodes
    modeAry[1] = max_freq; //returns Frequency 1
    
    //Fill the mode array
    cur_element = -1; // current element
    cur_freq = 0; // current element frequency
    int idx = 2; // current index in mode array
    ...
    for(int i = 0; i < arySize; ++i) {
        if(cur_element == ary[i]) { // like element
            ...
            if(cur_freq == max_freq) { // compare with max frequency
                modeAry[idx] = cur_element; // add mode elemnt to mode array
